...
export function GymnastProfileTab({
    ...
    }, [gymnast, editSection, getUpdateData, onGymnastUpdated]);
        ...
        <>
            ...
            <div className="card overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-3 bg-slate-50 border-b border-slate-200">
                    ...
                    <input ref={floorMusicInputRef} type="file" accept=".mp3,.wav,.aac,.m4a,.ogg,.flac,.wma" onChange={handleFloorMusicUpload} className="hidden" />
                </div>
                <div className="p-4">
                    {musicError && (<div className="mb-3 text-sm text-error-600 bg-error-50 px-3 py-2 rounded-lg">{musicError}</div>)}
                    {gymnast.floor_music_url ? (
                    ...
                    <div className="flex items-center gap-2 ml-auto">
                        ...
                        {editSection === 'medical' && (
                            <div className="flex items-center gap-1">
                                ...
                                <button onClick={saveSection} disabled={saving} className="p-1.5 rounded-md text-error-600 hover:text-error-700 hover:bg-error-100 transition-colors" title="Save">{saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5" />}</button>
                            </div>
                        )}
                    </div>
                </div>
                <div className="p-4">
                    ...
                    {canViewMedical && injuryReports.length > 0 && (
                        <div className="mt-4 pt-4 border-t border-error-100">
                            <button onClick={() => setShowInjuryHistory(!showInjuryHistory)} className="flex items-center gap-2 text-sm font-semibold text-error-600 hover:text-error-700 transition-colors w-full">
                                {showInjuryHistory ? (<ChevronDown className="h-4 w-4" />) : (<ChevronRight className="h-4 w-4" />)}
                            ...
                            {showInjuryHistory && (
                                <div className="mt-3 space-y-3">
                                    {injuryReports.map((injury) => (
                                        <div key={injury.id} className={`rounded-lg border p-3 ${injury.status === 'active' ? 'border-error-200 bg-error-50' : injury.status === 'recovering' ? 'border-amber-200 bg-amber-50' : 'border-success-200 bg-success-50'}`}>
