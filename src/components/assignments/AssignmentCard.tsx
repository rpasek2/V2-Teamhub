...
export function AssignmentCard({
    ...
    if (compact) {
        return (
            <div
                onClick={onClick}
                className={clsx(
                    'flex items-center gap-3 p-3 rounded-lg bg-surface border border-line',
                    onClick && 'cursor-pointer hover:border-accent-500 hover:bg-surface-hover transition-all',
                    className
                )}
            >
                <ProgressRing completed={progress.completed} total={progress.total} size={40} strokeWidth={3} />
                <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-heading truncate">{gymnastName}</p>
                    <p className="text-xs text-muted">{gymnastLevel}</p>
                </div>
    ...
    return (
        <div
            ...
            className={clsx(
                'card p-4 sm:p-5',
                onClick && 'cursor-pointer hover:border-accent-500/50 transition-all',
                className
            )}
        >
            {/* Header */}
            <div className="flex items-center justify-between gap-3 mb-4">
                <div className="flex items-center gap-3 min-w-0">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500/10 to-violet-500/10 flex items-center justify-center text-indigo-600 font-semibold flex-shrink-0 border border-indigo-500/20">
                        {gymnastName.charAt(0).toUpperCase()}
                    </div>
                    <div className="min-w-0">
                        <h3 className="font-semibold text-heading truncate">{gymnastName}</h3>
                        <p className="text-xs text-muted">{gymnastLevel}</p>
                    </div>
                </div>
                <div className="text-right flex-shrink-0">
                    <div className="text-2xl font-bold text-accent-600">{percentage}%</div>
                    <div className="text-xs text-muted">{progress.completed}/{progress.total}</div>
                </div>
            </div>
