...
export function StaffCertificationsSection({ staffUserId, canEdit }: StaffCertificationsSectionProps) {
    ...
    const getCertStatus = (expiryDate: string | null) => {
        ...
        if (isPast(expiry)) {
            return { status: 'expired', label: 'Expired', color: 'text-red-600 bg-red-500/10' };
        } else if (daysUntil <= 30) {
            return { status: 'expiring', label: `Expires in ${daysUntil} days`, color: 'text-amber-600 bg-amber-500/10' };
        } else if (daysUntil <= 90) {
            return { status: 'warning', label: `Expires in ${daysUntil} days`, color: 'text-orange-600 bg-orange-500/10' };
        }
        return { status: 'valid', label: 'Valid', color: 'text-green-600 bg-green-500/10' };
    };
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium text-heading">Certifications & Credentials</h3>
                {canEdit && !showAddForm && (
                    <button
            ...
            {showAddForm && (
                <div className="p-4 bg-surface-alt rounded-lg border border-line space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-body mb-1">Certification Name *</label>
                            <input
                                type="text"
                                value={name}
                        ...
                        <div>
                            <label className="block text-sm font-medium text-body mb-1">Issuing Organization</label>
                            <input
                                type="text"
                                value={issuer}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-body mb-1">Issue Date</label>
                            <input
                                type="date"
                                value={issueDate}
                        ...
                        <div>
                            <label className="block text-sm font-medium text-body mb-1">Expiry Date</label>
                            <input
                                type="date"
                                value={expiryDate}
                            ...
                            >
                                <div className="flex items-start justify-between">
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            <h4 className="font-medium text-heading">{cert.name}</h4>
                                            {cert.issuer && (
