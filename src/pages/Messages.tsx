...
export default function Messages() {
    ...
    return (
        <div className="flex h-[calc(100vh-4rem)] overflow-hidden bg-slate-800 shadow-sm ring-1 ring-slate-700/50 sm:rounded-xl">
            {/* Sidebar */}
            <div className="w-64 border-r border-slate-700/50 bg-slate-900 flex flex-col">
                <div className="p-4 border-b border-slate-700/50 flex justify-between items-center">
                    <h2 className="font-semibold text-chalk-50">Messages</h2>
                </div>
                <div className="flex-1 overflow-y-auto p-2 space-y-1">
                    ...
                    <div className="px-3 py-2 mt-4 text-xs font-semibold text-slate-400 uppercase tracking-wider flex items-center justify-between">
                        <span>Direct Messages</span>
                        <button
                            onClick={openNewDmModal}
                            className="p-1 rounded hover:bg-slate-700 transition-colors text-slate-400 hover:text-chalk-50"
                            title="New message"
                        >
                    ...
                    {dmChannels.length === 0 ? (
                        ...
                        dmChannels.map((channel) => (
                            ...
                            >
                                <div className="h-6 w-6 rounded-full bg-slate-700 flex items-center justify-center text-xs font-medium text-slate-300 mr-2">
                ...
                {selectedChannel ? (
                    <>
                        ...
                        <div className="px-6 py-4 border-b border-slate-700/50 flex items-center">
                            {selectedChannel.dm_participant_ids ? (
                                <>
                                    <div className="h-8 w-8 rounded-full bg-slate-700 flex items-center justify-center text-sm font-medium text-slate-300 mr-3">
                                        {selectedChannel.dm_other_user?.full_name?.[0] || '?'}
                                    </div>
                                    <h3 className="text-lg font-medium text-chalk-50">
                                ...
                                <>
                                    ...
                                    <h3 className="text-lg font-medium text-chalk-50">{selectedChannel.name}</h3>
                                    {selectedChannel.group_id && (
                                                ...
                                                <div>
                                                    {showHeader && (
                                                        <div className={`flex items-baseline mb-1 ${isMe ? 'justify-end' : 'justify-start'}`}>
                                                            <span className="text-sm font-medium text-chalk-50 mr-2">
                    ...
                    <div className="flex-1 flex items-center justify-center">
                        <div className="text-center">
                            <MessageSquare className="mx-auto h-12 w-12 text-slate-600" />
                            <h3 className="mt-2 text-sm font-semibold text-chalk-50">No conversation selected</h3>
                            <p className="mt-1 text-sm text-slate-400">Select a channel or start a new message.</p>
                        </div>
                    </div>
                )}
            </div>
            ...
            {showNewDmModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    />
                    <div className="relative w-full max-w-md bg-slate-800 rounded-xl shadow-2xl border border-slate-700/50">
                        <div className="flex items-center justify-between border-b border-slate-700/50 px-6 py-4">
                            <h3 className="text-lg font-semibold text-chalk-50">New Message</h3>
                            <button
                                onClick={() => setShowNewDmModal(false)}
                                className="rounded-full p-1 text-slate-400 hover:bg-slate-700 hover:text-chalk-50"
                            >
                                <X className="h-5 w-5" />
                            </button>
                        </div>
                        <div className="p-4">
                            ...
                            <div className="mt-4 max-h-64 overflow-y-auto">
                                {loadingMembers ? (
                                    <p className="text-center py-4 text-sm text-slate-400">Loading...</p>
                                ) : filteredMembers.length === 0 ? (
                                    <p className="text-center py-4 text-sm text-slate-400">No members found</p>
                                ) : (
                                    <div className="space-y-1">
                                        {filteredMembers.map((member) => (
                                            <button
                                                ...
                                                className="w-full flex items-center px-3 py-2 rounded-md hover:bg-slate-700 transition-colors"
                                            >
                                                <div className="h-8 w-8 rounded-full bg-slate-700 flex items-center justify-center text-sm font-medium text-slate-300 mr-3">
                                                    {member.full_name[0]}
                                                </div>
                                                <div className="text-left">
                                                    <p className="text-sm font-medium text-chalk-50">{member.full_name}</p>
